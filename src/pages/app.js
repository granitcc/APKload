...
function AppPages() {
  ...
  const sortArray = (type) => {
    ...
    sortedGame = sortedGame.filter(
      (d) => d.price >= price.min && d.price <= price.max
    );

    console.log(sortedGame);
    if (orderBy === "asc")
      sortedGame = sortedGame.sort((a, b) => (a[type] > b[type] ? 1 : -1));
    else sortedGame = sortedGame.sort((a, b) => (a[type] < b[type] ? 1 : -1));
        ...
        <div className="games-right">
          ...
          <div className="games-right-sort">
            ...
            <div className="games-right-sort-column-first">
              ...
              >
                <option className="option" value="names">
                  Names
                </option>
            ...
            {sortedGame.map((band) => (
              <div className="grc" key={band.id} style={{ margin: "10px" }}>
                ...
                <div className="rate">
                  {[...Array(5)].map((v, i) => {
                    return (
                      <FaStar className={`${i + 1 <= band.rate && "filled"}`} />
                    );
                  })}{" "}
                </div>
                <div>
